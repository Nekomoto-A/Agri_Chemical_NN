...
def select_features_by_lgbm(X, y, top_n):
    """
    LightGBMの重要度に基づいて特徴量を選択する関数
    
    Args:
        X (np.ndarray): 学習データ (n_samples, n_features)
        y (np.ndarray): ターゲットラベル (n_samples,)
        top_n (int): 選択したい特徴量の数
        
    Returns:
        X_selected (np.ndarray): 選択された特徴量のみのデータ
        selected_indices (np.ndarray): 選択された特徴量の列インデックス
    """
    
    # 1. LightGBM用のデータセット作成
    # 特徴量名がない場合は自動で column_0, column_1... となります
    ds = lgb.Dataset(X, label=y, free_raw_data=False)
    
    # 2. パラメータの設定 (基本的な回帰/分類の設定)
    # 目的関数などはデータの性質に合わせて調整可能ですが、
    # 重要度算出のためにはデフォルト設定でも十分に機能します。
    params = {
        'objective': 'regression', # 分類の場合は 'binary' や 'multiclass'
        'verbose': -1,
        'seed': 42
    }
    ...
    # 5. 重要度が高い順にインデックスをソートし、上位 top_n 個を取得
    # argsortは昇順なので [::-1] で降順にし、先頭から top_n 取る
    selected_indices = np.argsort(importances)[::-1][:top_n]
