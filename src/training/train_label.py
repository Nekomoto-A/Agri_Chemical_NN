...
def training_MT(x_tr,x_val,y_tr,y_val,model, optimizer, output_dim, reg_list, output_dir, model_name, label_tr = None,label_val = None,label_list = None, label_encoders = None, scheduler = None,
    ...
    for epoch in range(epochs):
        if visualize == True:
            if epoch == 0:
                ...
                if label_tr != None:
                    visualize_tsne(model = model, model_name = model_name , X = x_tr, Y = label_tr, reg_list = label_list, output_dir = output_dir, file_name = vis_name,X2 = x_val,Y2 = label_val)

        model.train()
        torch.autograd.set_detect_anomaly(True)
        outputs = model(x_tr,label_tr)
        train_losses = []
        ...
        if val == True:
            ...
            if visualize == True:
                if (epoch + 1) % 10 == 0:
                    ...
                    visualize_tsne(model = model, model_name = model_name , X = x_tr, Y = label_tr, reg_list = reg_list, output_dir = output_dir, file_name = vis_name, label_encoders = label_encoders,X2 = x_val,Y2 = label_val)
                    vis_losses = []
                    vis_losses_val = []
                    loss_list = []
