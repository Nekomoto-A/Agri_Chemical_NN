...
def training_MoE(x_tr,x_val,y_tr,y_val,model, output_dim, reg_list, output_dir, device, batch_size, #optimizer, 
                scalers, 
                train_ids, 
                reg_loss_fanction,
                lr = config['lr'], epochs = config['epochs'], early_stopping = config['early_stopping'],
                patience = config['patience'], tr_loss = config['tr_loss'], load_balancing_alpha = config['load_balancing_alpha'],
                ):
    ...
    for epoch in range(epochs):
        ...
        for x_batch, y_batch, masks_batch, patterns_batch in train_loader:
            ...
            outputs, _, load_balancing_loss =model(x_batch)
            #print(outputs)
            #print(y_batch)
            train_losses = {}
            ...
            learning_loss += load_balancing_alpha * load_balancing_loss
            ...
            running_train_losses['SUM'] += sum(l.item() for l in train_losses.values())
