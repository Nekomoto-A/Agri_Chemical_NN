...
def training_MT_DKL(x_tr,x_val,y_tr,y_val,model, reg_list, output_dir, 
    ...
    for epoch in range(epochs):
        ...
        for x_batch, label_batch, y_batch in train_loader:
            ...
            outputs, _, _ = model(x_batch, label_batch)
            train_losses = {}
            ...
            for i, reg in enumerate(reg_list):
                ...
                #output = outputs[reg]
                expert_outputs, gate_weights = outputs[reg]
                likelihood = model.likelihoods[i]
                ...
                #loss = -personal_losses[reg](output, true_tr.squeeze(-1), combined_input=combined_input).mean()
                loss = 0
                for j in range(model.gp_layers[i].num_experts):
                    # num_dataを指定することでミニバッチ学習に対応
                    mll = gpytorch.mlls.VariationalELBO(
                        likelihood, 
                        model.gp_layers[i].experts[j], 
                        num_data=len(train_loader.dataset)
                    )
                    
                    # 個別エキスパートのNegative ELBO
                    expert_elbo = -mll(expert_outputs[j], true_tr)
                    
                    # ゲート重みで加重平均
                    # 各データポイントの重みを反映させるため、要素ごとに掛けてから平均をとる
                    weighted_elbo = (gate_weights[:, j] * expert_elbo).mean()
                    loss += weighted_elbo
        ...
        if val == True:
            ...
            with torch.no_grad():
                for x_val_batch, label_val_batch, y_val_batch in val_loader:
                    ...
                    outputs, _, _ = model(x_val_batch, label_val_batch)
                    val_losses = []
                    ...
                    for i, reg in enumerate(reg_list):
                        ...
                        loss = 0
                        for j in range(model.gp_layers[i].num_experts):
                            mll = gpytorch.mlls.VariationalELBO(likelihood, model.gp_layers[i].experts[j], num_data=len(val_loader.dataset))
                            expert_elbo = -mll(expert_outputs[j], true_val)
                            loss += (gate_weights[:, j] * expert_elbo).mean()
    ...
    with torch.no_grad():
        ...
        for x_tr_batch, label_tr_batch, y_tr_batch in train_loader:
            ...
            for k, target in enumerate(reg_list):
                # 1. 正解ラベルの格納 (変更なし)
                ...
                expert_dists, weights = outputs[reg]
                # モーメントマッチングによる統合
                m_list, v_list = [], []
                for n in range(len(expert_dists)):
                    prediction = model.likelihoods[k](expert_dists[n])
                    m_list.append(prediction.mean)
                    v_list.append(prediction.variance)
                
                means = torch.stack(m_list, dim=-1)
                vars = torch.stack(v_list, dim=-1)
                
                batch_mean = (weights * means).sum(dim=-1)
                batch_var = (weights * (vars + means**2)).sum(dim=-1) - batch_mean**2

                pred.setdefault(target, []).append(batch_mean.cpu().detach().numpy())
