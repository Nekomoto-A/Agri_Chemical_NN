...
class DensityWeightedDataset(Dataset):
    def __init__(self, inputs, targets_dict, primary_reg_key):
        """
        Args:
            inputs: 入力データ (Tensor)
            targets_dict: 目的変数の辞書 {reg_name: Tensor}
            primary_reg_key: 密度計算の基準とする目的変数のキー (例: reg_list[0])
        """
        self.inputs = inputs
        self.targets_dict = targets_dict
        
        # 1. 密度に基づく重みの計算
        primary_targets = targets_dict[primary_reg_key].numpy().flatten()
        kde = gaussian_kde(primary_targets)
        densities = kde(primary_targets)
        
        # 密度の逆数を重みにする (小さい密度 = 大きい重み)
        weights = 1.0 / (densities + 1e-6)
        # 平均を1に正規化して安定させる
        self.weights = torch.tensor(weights / np.mean(weights), dtype=torch.float32)

    def __len__(self):
        return len(self.inputs)

    def __getitem__(self, idx):
        # 入力、全ターゲットの辞書、およびそのデータの重みを返す
        target_sample = {k: v[idx] for k, v in self.targets_dict.items()}
        return self.inputs[idx], target_sample, self.weights[idx]
...
def training_MT_FDS(x_tr,x_val,y_tr,y_val,model, output_dim, reg_list, output_dir, model_name,loss_sum, device, batch_size, #optimizer, 
                scalers, 
                train_ids, 
                reg_loss_fanction,
    ...
    train_dataset = DensityWeightedDataset(x_tr, y_tr, reg_list[0])
    ...
    val_dataset = DensityWeightedDataset(x_val, y_val, reg_list[0])
    ...
    for epoch in range(epochs):
        ...
        for x_batch, y_batch, weights_batch in train_loader:
            ...
            #weights_batch = {k: v.to(device) for k, v in weights_batch}
            #masks_batch = {k: v.to(device) for k, v in masks_batch.items()}
            
            model.train()
            optimizer.zero_grad()

            #outputs, encoded_features = model(x_batch)
            encoded_features = model.shared_block(x_batch)
            train_losses = {}

            if FDS == 'DWMixup':
                from src.training.latent_smooth import apply_density_weighted_mixup
                ds_features, ds_targets = apply_density_weighted_mixup(features = encoded_features, 
                                                                        targets_dict = y_batch, 
                                                                        weights_batch = weights_batch
                                                                        )

            for reg, lf, head in zip(reg_list, reg_loss_fanction, model.task_specific_heads):
                output = head(ds_features)

                loss = personal_losses[reg](output, ds_targets[reg])
    ...
    with torch.no_grad():
        true = {}
        pred = {}
        for x_tr_batch, y_tr_batch, _ in train_loader:
