...
def training_TabPFN_shuffle(x_tr,x_val,y_tr,y_val,models, labels_train,labels_val, reg_list, scalers, output_dir, 
    ...
    for reg in reg_list:
        ...
        # hist_dir = os.path.join(train_dir, f'hist_train_{reg}.png')
        # save_labeled_histograms(data = y_tr[reg], labels = labels_train, save_path = hist_dir)


        # hist_dir = os.path.join(train_dir, f'hist_train_{reg}_cluster.png')
        # save_labeled_histograms(data = y_tr[reg], labels = np.array(result_detail['Assigned_Cluster']), save_path = hist_dir)
