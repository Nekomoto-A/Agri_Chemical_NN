...
def training_MT_WGP(x_tr,x_val,y_tr,y_val,model, reg_list, output_dir, 
    ...
    {'params': model.gp_layers.parameters(), 'lr': lr},
    {'params': model.likelihoods.parameters(), 'lr': lr},
    {'params': model.warping_layers.parameters(), 'lr': lr}, # これを追加！
    # もしエンコーダーも学習するならここに追加
    {'params': model.shared_block.parameters(), 'lr': lr * 0.1}, 
    ], lr=lr)
    ...
    for epoch in range(epochs):
        ...
        for x_batch, label_batch, y_batch in train_loader:
            ...
            for i, reg in enumerate(reg_list):
                y_target = y_batch[reg].to(device)
                # ❶ 正解ラベルとモデルの出力を取得
                # 2. ターゲット y を z に変換
                true_tr = model.warping_layers[i](y_target)
                ...
                log_jacobian = model.warping_layers[i].get_log_jacobian(y_target).squeeze(-1)
                correction = -log_jacobian.mean()
                ...
                # .detach() して numpy に変換（要素が多い場合はこれが見やすい）
                # ls_values = model.gp_layers[i].covar_module.base_kernel.lengthscale.detach().cpu().numpy()
                # print(f"Task {reg} Lengthscales: {ls_values}")
        ...
        if val == True:
            ...
            with torch.no_grad():
                for x_val_batch, label_batch, y_val_batch in val_loader:
                    ...
                    for i, reg in enumerate(reg_list):
                        y_target_val = y_val_batch[reg].to(device)

                        true_val = model.warping_layers[i](y_target_val)
                        ...
                        val_log_jacobian = model.warping_layers[i].get_log_jacobian(y_target_val).squeeze(-1)
                        val_correction = -val_log_jacobian.mean()
    ...
    with torch.no_grad():
        ...
        for x_tr_batch, label_tr_batch, y_tr_batch in train_loader:
            ...
            outputs = model.predict(x_tr_batch)

            for i, target in enumerate(reg_list):
                # 1. 正解ラベルの格納 (変更なし)
                ...
                z_means = outputs[target]['z_space_mean']

                y_train_min = y_tr[target].min().item()
                y_train_max = y_tr[target].max().item()
                margin = (y_train_max - y_train_min) * 0.5 # 50%のマージン

                y_min = y_train_min - margin
                y_max = y_train_max + margin
                
                raw_output = inverse_warping(model.warping_layers[i], z_means, y_min, y_max).cpu().detach()
...
def inverse_warping(warping_layer, z_pred, y_min=-10, y_max=10, iterations=50):
    """
