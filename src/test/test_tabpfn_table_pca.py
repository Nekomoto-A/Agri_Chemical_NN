...
def test_tabpfn_pca(model, pca, X_test, y_test, X_train, Y_train, reg, output_dir, result_dir, 
                index, model_name, 
    ...
    X_test = pd.DataFrame(pca.transform(X_test), columns=X_train.columns)
...
from src.training.training_TabPFN_table_pca import train_tabpfn_pca

def train_and_test_tabpfn_pca(X_train, Y_train, X_test, Y_test, reg, output_dir, result_dir, eval_reg, eval_class, index, model_name, scalers = None, shap_compute = False, label_encoders = None):
    model, pca, X_train_pca = train_tabpfn_pca(X_train, Y_train, reg, output_dir, scalers)
    score, model, true, pred= test_tabpfn_pca(model, pca, X_test, Y_test, X_train_pca, Y_train, reg, output_dir, result_dir, index, model_name, eval_reg, eval_class, scalers, shap_compute, label_encoders)
