...
def test_MT_DKL(x_te, label_te, y_te, model, reg_list, scalers, output_dir, device, test_ids, n_samples_mc=100):
    ...
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        shared_features = model.shared_block(x_te)
        combined_input = torch.cat([shared_features, label_te], dim=-1)
        for i, reg in enumerate(reg_list):
            if torch.is_floating_point(y_te[reg]):
                expert_dists, weights = model.gp_layers[i](combined_input)
                
                # --- 混合分布のモーメントマッチング ---
                # 1. 各エキスパートの予測（観測ノイズ込み）
                means = []
                variances = []
                for j in range(len(expert_dists)):
                    pred = model.likelihoods[i](expert_dists[j])
                    means.append(pred.mean)      # [batch_size]
                    variances.append(pred.variance) # [batch_size]
                
                # スタックして [batch_size, num_experts] に変換
                means = torch.stack(means, dim=-1)
                variances = torch.stack(variances, dim=-1)
                
                # 2. 混合平均: E[y] = Σ w_i * μ_i
                mixed_mean = (weights * means).sum(dim=-1)
                
                # 3. 混合分散: Var[y] = Σ w_i * (σ_i^2 + μ_i^2) - E[y]^2
                # これは「各エキスパートの分散の平均」＋「平均値のばらつき」を表します
                mixed_var = (weights * (variances + means**2)).sum(dim=-1) - mixed_mean**2        
                
                pred_mean_tensor = mixed_mean
                pred_std_tensor = torch.sqrt(mixed_var.clamp(min=1e-6))
