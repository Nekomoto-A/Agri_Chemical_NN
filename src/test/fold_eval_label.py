from src.datasets.dataset_label import data_create,transform_after_split

from sklearn.model_selection import KFold
import os
import shutil
from src.test.test_label import train_and_test,write_result
...
def fold_evaluate(reg_list, label_list, 
    ...
    sub_dir = os.path.join(output_dir, f'目的変数：{reg_list}, ラベル：{label_list}')
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X, Y)):
        ...
        X_train_tensor, X_val_tensor, X_test_tensor, label_train_tensor, label_val_tensor, label_test_tensor, Y_train_tensor, Y_val_tensor, Y_test_tensor,scalers = transform_after_split(X_train,X_test,Y_train,Y_test, reg_list = reg_list, label_list = label_list)
        
        #label_train_tensor, label_val_tensor, label_test_tensor = label_train_tensor[0], label_val_tensor[0], label_test_tensor[0]
        ...
        predictions, trues, r2_results, mse_results,model_trained = train_and_test(
            ...
            vis_dir = vis_dir, model_name = model_name, label_encoders = label_encoders,
            label_train = label_train_tensor, label_val = label_val_tensor, label_test = label_test_tensor, 
            label_list = label_list
            )
        
        '''
        reduced_features = model_trained.sharedconv(X_test_tensor).detach().numpy()
        ...
        for reg in reg_list:
            ...
            if Y is not None:
                if np.issubdtype(y.dtype, np.integer):  
                    ...
                    scatter = plt.scatter(x[:, 0], x[:, 1], c=label_encoders[reg].inverse_transform(y), cmap='tab10')
