...
for rank in ranks:
    ...
    for train_ix, test_ix in cv_outer.split(X):
        ...
        search = GridSearchCV(model, space, scoring='accuracy', n_jobs=40, cv=cv_inner, refit=True)
        # execute search
        ...
        pickle.dump(model, open(os.path.join(dir, filename), 'wb'))
        
        idx += 1

    # summarize the estimated performance of the model
    with open(os.path.join(dir, f'performance_{rank}_{ml_model}.txt'), 'w') as file:
