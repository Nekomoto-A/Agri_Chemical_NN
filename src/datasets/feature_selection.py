...
def select_features_with_vif_report(X, output_dir, thresh=10.0, csv_dir="original_vif_report.csv"):
    """
    VIFを計算してCSV保存し、その後多重共線性の高い特徴量を順次削除する関数
    """

    csv_path = os.path.join(output_dir, csv_dir)
    
    # --- 1. オリジナルデータのVIFを計算・保存 ---
    print("Calculating original VIF scores...")
    X_const_init = add_constant(X)
    
    # 各カラムのVIFを計算（定数項'const'以外）
    initial_vif = pd.Series(
        [variance_inflation_factor(X_const_init.values, i) 
         for i in range(X_const_init.shape[1])],
        index=X_const_init.columns
    ).drop('const', errors='ignore')
    
    # CSVとして保存
    initial_vif.to_csv(csv_path, header=["VIF"])
    print(f"Original VIF report saved to: {csv_path}")
    ...
    while True:
        X_with_const = add_constant(current_X[cols])
        ...
        if max_vif > thresh:
            ...
            print(f"Removing '{max_col}' (VIF: {max_vif:.2f})")
            cols.remove(max_col)
        else:
            break
