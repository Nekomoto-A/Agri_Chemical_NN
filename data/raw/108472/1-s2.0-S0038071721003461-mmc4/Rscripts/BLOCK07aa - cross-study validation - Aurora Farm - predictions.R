...
for (permute_labels in c("permuted","real")){
  ...
  # Aurora data lacks information on the following measures:
    ...
    # calculate pearsons
    r <- rcorr(unlist(predictions), dataTransformed[,factor], type = "pearson")$r[1,2]
    p <- rcorr(unlist(predictions), dataTransformed[,factor], type = "pearson")$P[1,2]
    
    # get coefficient of determination (Rsquared)
    Rsqr <- summary(lm(unlist(predictions) ~ dataTransformed[,factor]))$r.squared
    
    # get mean square error
    mse <- mean((summary(lm(unlist(predictions) ~ dataTransformed[,factor]))$residuals)^2)
    
    if (count == 1){
      results <- data.frame(mod = mo, see = seed, dataset = dataset, fact = factor, feat = feature_type, tax_rank = rank, pearsons.r = r, p.value = p, R.squared = Rsqr, MSE = mse, datatype = permute_labels, stringsAsFactors = F)
      results2 <- data.frame(mod = mo, see = seed, dataset = dataset, fact = factor, feat = feature_type, tax_rank = rank, aurora.sample = row.names(dataTransformed), aurora.data = dataTransformed[,factor], prediction.data = unlist(predictions), datatype = permute_labels, stringsAsFactors = F)
      results3 <- rbind.fill(data.frame(mod = mo, see = seed, dataset = dataset, fact = factor, feat = feature_type, tax_rank = rank, test.missing = setdiff(features, taxa_names(p)), datatype = permute_labels, stringsAsFactors = F), data.frame(mod = mo, see = seed, dataset = dataset, fact = factor, feat = feature_type, tax_rank = rank, model.missing = model.miss, datatype = permute_labels, stringsAsFactors = F))
      count <- count + 1
    } else {
      results <- rbind(results, data.frame(mod = mo, see = seed, dataset = dataset, fact = factor, feat = feature_type, tax_rank = rank, pearsons.r = r, p.value = p, R.squared = Rsqr, MSE = mse, datatype = permute_labels, stringsAsFactors = F))
      results2 <- rbind(results2, data.frame(mod = mo, see = seed, dataset = dataset, fact = factor, feat = feature_type, tax_rank = rank, aurora.sample = row.names(dataTransformed), aurora.data = dataTransformed[,factor], prediction.data = unlist(predictions), datatype = permute_labels, stringsAsFactors = F))
      results3 <- rbind(results3, rbind.fill(data.frame(mod = mo, see = seed, dataset = dataset, fact = factor, feat = feature_type, tax_rank = rank, test.missing = setdiff(features, taxa_names(p)), datatype = permute_labels, stringsAsFactors = F), data.frame(mod = mo, see = seed, dataset = dataset, fact = factor, feat = feature_type, tax_rank = rank, model.missing = model.miss, datatype = permute_labels, stringsAsFactors = F)))        
    }
  }
}

colnames(results) <- c("model","seed","dataset","health.metric","feature_type","rank","pearsons.r","p.value","R.squared","MSE","test.set")
colnames(results2) <- c("model","seed","dataset","health.metric","feature_type","rank","aurora.sampleID","observed","predicted","test.set")
colnames(results3) <- c("model","seed","dataset","health.metric","feature_type","rank","missing.from.model","test.set","missing.from.testset")
results3 <- subset(results3, test.set == "real")

saveRDS(results, file = "data/aurora.correlations.final.rds")
saveRDS(results2, file = "data/aurora.predictions.final.rds")
saveRDS(results3, file = "data/aurora.missing.features.final.rds")
