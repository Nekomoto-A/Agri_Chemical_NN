...
perm_test <- function(file){
  ...
  base_metric <- summary(lm(predictions ~ dataTransformed[,factor]))$r.squared
  otus <- colnames(dataTransformed[-which(colnames(dataTransformed) == factor)])
...
bestmodels <-  readRDS(file = "../microbiome.data/aurora.correlations.final.rds")
bestmodels <- subset(bestmodels, health.metric %in% c("avg.rating","DNA","ace_soil_protein_index_rating","respiration_rating","active_carbon_rating","organic_matter_rating","aggregate_stability_rating"))
bestmodels <- subset(bestmodels, test.set == "real" & pearsons.r > 0 & dataset == "minimal.norm" & rank == "ASV")
...
# save backup
saveRDS(results, file = paste("data/aurora.feature.importance.rds",sep=""))


##################################
## Package results and save output

results <- readRDS(file = "../models/aurora.feature.importance.rds")
...
# Save
saveRDS(perm_results, file = "data/aurora.feature.importance.final.rds")